    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for precond_A in actionA.preconditions:
            for precond_B in actionB.preconditions:
                if self.parent_layer.is_mutex(precond_A, precond_B) & self.parent_layer.is_mutex(precond_B, precond_A):
                    continue
                else:
                    return False
        return True
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        """
        # TODO: implement this function
        for achieve_A in self.parents[literalA]:
            for achieve_B in self.parents[literalB]:
                if self.parent_layer.is_mutex(achieve_A, achieve_B) & self.parent_layer.is_mutex(achieve_B, achieve_A):
                    continue
                else:
                    return False 
        return True
    ...
    def _negation(self, literalA, literalB):
        ...
        # TODO: implement this function
        return (literalA == ~literalB) & (literalB == ~literalA)
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        level_sum = 0
        level = 0
        set_goals = set(self.goal)
                                                                          
        while not self._is_leveled:
            level_goals = set()
            for goal in set_goals:
                if (goal in self.literal_layers[-1]):
                    level_goals.add(goal)
            set_goals.difference_update(level_goals)
            level_sum += level * len(level_goals)           
            if len(set_goals) == 0:
                 break
            else:
                self._extend()
    ...
    def h_maxlevel(self):
        ...
        while not self._is_leveled:
            level_goals = set(self.literal_layers[-1])                                                            
            if set_goals.issubset(level_goals):
                level_max = level
                break
            else:
                self._extend()
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:        
            ...
            level_goals = self.literal_layers[-1]
            no_pair_mutex = True
            
            if set_goals.issubset(level_goals):

                for goal_A, goal_B in combinations(set_goals,2):
                    if level_goals.is_mutex(goal_A, goal_B):
                        no_pair_mutex = False
                        break
